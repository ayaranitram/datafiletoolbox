...
def PercentualDifference( SimResultObjects=[] , AttributesOrKeys=[] , TimeSteps=None , Interpolate=False ) :
    ...
    for each in AttributesOrKeys :
        if ':' in each :
            ...
            MatchingKeys.append(each.strip())
            _verbose( verbosity , 0 , ' < %Diff >   ' + each.strip() )
    _verbose( verbosity , 1 , ' < %Diff > found ' + str(len(MatchingKeys)) + ' Keys and ' + str(len(Atts)) + ' Attributes,')
    ...
    _verbose( verbosity , 1 , ' < %Diff > totalizing ' + str(len(MatchingKeys)) + ' Keys after extending the Attributes and removing duplicates.')

    if TimeSteps is None :
        _verbose( verbosity , 1 , ' < %Diff > TimeSteps not received, all the matching timesteps will be used.')
        TimeSteps = list()
        for obj in SimResultObjects :
            _verbose( verbosity , 0 , ' < %Diff > getting TimeSteps from ' + str(obj.get_Name()) + '\n < %Diff > extracted ' + str(len( list( obj.get_Vector( TimeKind )[ TimeKind ] ))) + ' ' + TimeKind + '.' )
        ...
        _verbose( verbosity , 1 , ' < %Diff > ' + str(len(TimeSteps)) + ' TimeSteps found, from ' + str(TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.' )
                
    else :
        _verbose( verbosity , 1 , ' < %Diff > ' + str(len(TimeSteps)) + ' TimeSteps received, from ' + str(TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.' )
        ...
        if Interpolate == False :
            ...
            if len(TimeSteps) > 0 :
                _verbose( verbosity , 1 , ' < %Diff > ' + str(len(TimeSteps)) + ' TimeSteps found matching the simulation data TimeSteps, from ' + str(TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.' )
            else :
            ...
            if len(TimeSteps) - len(MatchingTimeSteps) > 0 :
                _verbose( verbosity , 2 , ' < %Diff > WARNING: data will be interpolated for ' + str( len(TimeSteps) - len(MatchingTimeSteps) ) + ' TimeSteps not found in the simulation results.\n < %Diff > WARNING: Set Interpolate=False to avoid this behaviour.' )
    ...
    _verbose( verbosity , -1 , ' < %Diff >  resulting Percentual Difference is: ' + str( abs(sumDF).mean(axis=1).mean(axis=0) ) )
